...
def _stabilize_plan(
    ...
    for entry in photo_views:
        ...
        if photo_type in ("interior", "auxiliary"):
            view_id = photo_type
        elif photo_type == "close_up_damage":
            # close_up_damage is only a photo_type; the planner still needs to assign
            # an exterior view. Keep the planner's view unless it is already interior/
            # auxiliary, otherwise fall through to scene_intake.
            if view_id in NON_EXTERIOR_VIEWS and view_id not in ("interior", "auxiliary"):
...
async def planner_agent(
    ...
    # Last resort: if known entries are too sparse or the merged plan has fewer
    # than 6 exterior views, spend one LLM call to recover. JSON mode is
    # explicitly enabled because legacy call_minimax defaults do not include it.
    known_entries_count = sum(1 for e in photo_views if e["view_id"] not in ("unknown", ""))
    known_ratio = known_entries_count / total_input_photos if total_input_photos else 1.0
    if known_ratio < 0.5 or len(covered_views) < 6:
        logger.warning(
            "[planner] sparse plan: known_ratio=%.2f (%d/%d) covered_views=%s; running fallback_replan",
            known_ratio, known_entries_count, total_input_photos, covered_views,
        )
