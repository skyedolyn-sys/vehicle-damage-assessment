...
def build_summary(result_native: dict, elapsed: float) -> dict:
    ...
    exterior_views = {'front', 'front_left', 'front_right', 'rear', 'rear_left', 'rear_right', 'left', 'right'}
    covered_views = [v for v, g in view_groups.items() if g and v in exterior_views]

    # Aggregate per-view part counts
    subagent_results = result_native.get('subagent_results', [])
    subagent_summaries = []
    for sr in subagent_results:
        view_id = sr.get('view_id', 'unknown')
        part_count = len(sr.get('parts', []))
        subagent_summaries.append({"view_id": view_id, "part_count": part_count})

    # Track any anomalies / retries via review
    review = result_native.get('review', {})
    ...
    return {
        ...
        "subagent_count": len([v for v, g in view_groups.items() if g and v in exterior_views]),
...
async def run_once(run_id: int):
    ...
    out_path = os.path.join(OUT_DIR, f'team_v4_run_{run_id}.json')
    with open(out_path, 'w', encoding='utf-8') as f:
        json.dump(result_native, f, ensure_ascii=False, indent=2)

    summary = build_summary(result_native, elapsed)
    summary_path = os.path.join(OUT_DIR, f'team_v4_run_{run_id}_summary.json')
    with open(summary_path, 'w', encoding='utf-8') as f:
        json.dump({"run": run_id, **summary}, f, ensure_ascii=False, indent=2)

    print(f"Run {run_id}: saved to {out_path}, elapsed={elapsed:.1f}s, uncertain={summary['uncertain_count']}, views={summary['covered_view_count']}")
    return summary
