...
async def main():
    ...
    print(f"Step 2: Photo Locator ({len(photos)} photos, batch={PHOTO_LOCATOR_BATCH_SIZE}, concurrent={MAX_CONCURRENT_API_CALLS})...")
    t2 = time.time()
    ...
    print(f"  done in {time.time()-t2:.1f}s, locations: {len(all_locations)}")

    print("Step 3: Regional Workers + Synthesizer...")
    t3 = time.time()
    damage_result = await damage_assessor_agent(photos, all_locations, vehicle_prior)
    print(f"  done in {time.time()-t3:.1f}s")
    print(f"  parts: {len(damage_result.get('parts', []))}")

    print(f"\nTotal: {time.time()-start:.1f}s")

    with open("debug_regional.json", "w", encoding="utf-8") as f:
